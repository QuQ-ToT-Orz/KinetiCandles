...
class KinetiCandlesApp:
    ...
    def update_view_label(self) -> None:
        ...
        if self.current_view == "day":
            self.view_label.config(text=f"Viewing: {current_date.strftime('%A, %B %d, %Y')} (Daily)")
        elif self.current_view == "week":
    ...
    def toggle_simulation(self) -> None:
        ...
        if self.simulation_active:
            ...
            # Run simulation using Tkinter's event scheduling
            self.run_simulation()
            
            # Provide feedback
            messagebox.showinfo("Simulation Started", 
                              "Simulation mode will cycle through all days in the dataset.\n\n"
                              "Each day will be shown for 3 seconds before moving to the next day.\n\n"
                              "Click 'Stop Simulation' to end the simulation at any time.")
    ...
    def run_simulation(self) -> None:
        ...
        self.update_view_label()  # Update view label immediately
        self.update_view()  # Force an initial view update
        
        if not self.model.days:
            return
            
        start_day = self.model.current_day_index
        
        # Define a function to update a single day and schedule the next one
        def update_day():
            ...
            if self.simulation_active:
                ...
                if self.model.current_day_index == start_day:
                    self.simulation_active = False
                    self.sim_btn.config(text="Start Simulation")
                else:
                    # Schedule the next update after 3 seconds
                    self.root.after(3000, update_day)
        
        # Start the simulation
        self.root.after(500, update_day)  # Start after a short delay
